...
java -Xms512M -Xmx2G -Xss2M -XX:ReservedCodeCacheSize=192m -Dfile.encoding=UTF-8 -jar sbt/sbt-launch.jar -Dsbt.parser.simple=true clean +test +common/test +macroCommon/test +macroSub/test +core/test +scalatest/test 'set every publishTo := Some(Resolver.file("local-repo", file("target/dist")))' '+ publish'
