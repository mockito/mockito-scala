...
/**
 * Low-priority willBe answered extensions for generic R values (non-function types). Function-specific extensions in IdiomaticMockitoCats take priority over these.
 */
private[mockito] trait LowPriorityCatsDoSomething {
...
/**
 * Scala 3 version of IdiomaticMockitoCats with inline macro-based stubbing operations.
 */
private[mockito] trait IdiomaticMockitoCats extends IdiomaticMockitoCatsRuntime with LowPriorityCatsDoSomething {
  import org.mockito.cats.IdiomaticMockitoCats.*
  ...
  extension [F[_], T](inline stubbing: F[T]) {
    transparent inline def shouldReturnF: ReturnActions[F, T] =
      new ReturnActions[F, T](CatsStubbing(WhenMacro.whenRaw[F[T]](stubbing)))
    ...
    transparent inline def shouldFailWith: ThrowActions[F, T] =
      new ThrowActions[F, T](CatsStubbing(WhenMacro.whenRaw[F[T]](stubbing)))
    ...
    transparent inline def shouldAnswerF: AnswerActions[F, T] =
      new AnswerActions[F, T](CatsStubbing(WhenMacro.whenRaw[F[T]](stubbing)))
  ...
  extension [F[_], G[_], T](inline stubbing: F[G[T]]) {
    transparent inline def shouldReturnFG: ReturnActions2[F, G, T] =
      new ReturnActions2[F, G, T](CatsStubbing2(WhenMacro.whenRaw[F[G[T]]](stubbing)))
    ...
    transparent inline def shouldFailWithG: ThrowActions2[F, G, T] =
      new ThrowActions2[F, G, T](CatsStubbing2(WhenMacro.whenRaw[F[G[T]]](stubbing)))
    ...
    transparent inline def shouldAnswerFG: AnswerActions2[F, G, T] =
      new AnswerActions2[F, G, T](CatsStubbing2(WhenMacro.whenRaw[F[G[T]]](stubbing)))
  ...
  // Function-specific extensions (higher priority than generic R in LowPriorityCatsDoSomething).
...
object IdiomaticMockitoCats extends IdiomaticMockitoCats {
  ...
  case class RaisedBy[T](value: T) {
    transparent inline def by[F[_], E](inline stubbing: F[E])(using F: ApplicativeError[F, ? >: T]): F[E] =
  ...
  case class RaisedByG[T](value: T) {
    transparent inline def by[F[_], G[_], E](inline stubbing: F[G[E]])(using F: Applicative[F], G: ApplicativeError[G, ? >: T]): F[G[E]] =
