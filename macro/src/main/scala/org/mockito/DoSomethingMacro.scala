...
object DoSomethingMacro {
  ...
  def answeredBy[T: c.WeakTypeTag](c: blackbox.Context)(stubbing: c.Expr[T]): c.Expr[T] = {
    ...
    c.Expr[T] {
      c.macroApplication match {
        case q"$_.DoSomethingOps[$r]($v).willBe($_.answered).by[$_]($obj.$method(..$args))" =>
          if (args.exists(a => isMatcher(c)(a))) {
            val newArgs: Seq[Tree] = args.map(a => transformArg(c)(a))
            q"org.mockito.Mockito.doAnswer(org.mockito.DoSomethingMacro.argumentToAnswer($v)).when($obj).$method(..$newArgs)"
          } else
            q"org.mockito.Mockito.doAnswer(org.mockito.DoSomethingMacro.argumentToAnswer($v)).when($obj).$method(..$args)"
        ...
        case q"$_.DoSomethingOps[$r]($v).willBe($_.answered).by[$_]($obj.$method)" =>
          q"org.mockito.Mockito.doAnswer(org.mockito.DoSomethingMacro.argumentToAnswer($v)).when($obj).$method"
